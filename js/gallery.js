...
const imageEl = link.querySelector('.gallery-image');
const largeSrc = imageEl.dataset.source;
...
const instance = basicLightbox.create(
    ...
    {
    onShow: (inst) => {
        const onEsc = (ev) => ev.key === 'Escape' && inst.close();
        document.addEventListener('keydown', onEsc);
        inst.element()._esc = onEsc;
    },
    onClose: (inst) => {
        document.removeEventListener('keydown', inst.element()._esc);
    }
    }
);
